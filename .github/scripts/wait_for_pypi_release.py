...
message about a cancelled job where this was written to name what to go
and read (issue btclib-org/btclib#1165). Every request is bounded by
what is left of the deadline as well as by its own timeout, so the whole
wait ends within `--timeout` of its first request for anything the index
...
applies per blocking read, so that case is the job's `timeout-minutes`
to end.
