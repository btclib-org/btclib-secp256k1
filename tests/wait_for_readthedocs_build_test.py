...
_URL = "https://btclib-secp256k1.readthedocs.io/en/v0.8.0.5/"
_BUILDS = "https://app.readthedocs.org/projects/btclib-secp256k1/builds/"
_AGENT = "btclib-secp256k1-readthedocs-wait/1.0"
...
@contextmanager
def _not_found() -> Iterator[Exception]:
    """Refuse the way the site refuses a version it has not built.

    Closed rather than built and dropped: `HTTPError` given no file
    object makes one of its own, through the temporary-file wrapper
    `urllib.response.addbase` inherits, and the deallocator of that
    wrapper warns `ResourceWarning`. `filterwarnings = ["error"]` turns
    that into a failure of whichever test is running when the object is
    collected, which is not this one (btclib-org/btclib#1550).
    """
    with closing(
...
@contextmanager
def _timed_out() -> Iterator[Exception]:
    """Refuse the way a connection the site never answers on does."""
    yield TimeoutError("timed out")
...
def test_a_build_that_has_not_finished_is_waited_for(
    ...
    assert script.main(["btclib-secp256k1", "v0.8.0.5", "--interval", "30"]) == 0
...
def test_a_build_that_never_arrives_is_the_deadline_speaking(
    ...
    assert script.main(["btclib-secp256k1", "v0.8.0.5", "--timeout", "60"]) == 1

    assert clock.now == pytest.approx(60.0)
    assert len(site.asked) == 2
    reported = capsys.readouterr().out
    assert f"::error::tag v0.8.0.5: {_URL} was never served in 60 s;" in reported
...
def test_a_deadline_already_spent_asks_nothing_and_still_annotates(
    ...
    assert script.main(["btclib-secp256k1", "v0.8.0.5", "--timeout", "0"]) == 1
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert script.main(["btclib-secp256k1", "v0.8.0.5", "--timeout", "30"]) == 1
...
def test_the_request_bound_is_cut_to_what_is_left_of_the_deadline(
    ...
    `clock` is asked for although nothing here reads it, and that is the
    point: without it a regressed guard falls through to the real
    `time.sleep` and the test sits for the whole interval before
    reporting anything, where the fake turns the same regression into an
    immediate failure. It is a `usefixtures` mark rather than an argument
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    assert script.main(["btclib-secp256k1", "v0.8.0.5"]) == 1
...
def test_the_site_answering_the_page_is_it_serving_the_tag(
    ...
    agent = script.USER_AGENT.format(project="btclib-secp256k1")
    assert agent == _AGENT
...
def test_a_status_the_site_refuses_with_is_named_by_its_number(
    ...
    """
    with _not_found() as failure:

        def urlopen(
            *_args: object, **_kwargs: object
        ) -> AbstractContextManager[_Answer]:
            """Refuse the way the site refuses a version it has not built."""
            raise failure
...
def test_the_main_guard_runs_the_script_as___main__(
    ...
    the file fresh with `__name__` set to `"__main__"` in this one, on a
    deadline of zero, which is the one argument that asks the site
