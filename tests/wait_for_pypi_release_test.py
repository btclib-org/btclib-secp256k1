...
def _not_found() -> Iterator[Exception]:
    """Refuse the way the index refuses while the upload is still landing.
...
def _timed_out() -> Iterator[Exception]:
    """Refuse the way a connection the index never answers on does."""
...
def test_the_version_asked_for_is_the_one_the_tag_names(
    ...
    assert clock.slept == []
    assert "the index serves 0.8.0.5" in capsys.readouterr().out
...
def test_a_version_not_served_yet_is_waited_for(
    ...
    assert "the index serves 0.8.0.5 after 30 s" in capsys.readouterr().out
...
def test_a_version_that_never_arrives_is_the_deadline_speaking(
    ...
    assert "::error::0.8.0.5 is not on the index 300 s later" in reported
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert "::error::0.8.0.5 is not on the index 30 s later" in capsys.readouterr().out
...
def test_a_2xx_that_is_not_ok_is_not_the_index_serving_it(
    ...
    mutation that survives it. This is the case that kills that one.
    """

    def urlopen(*_args: object, **_kwargs: object) -> AbstractContextManager[_Answer]:
        """Answer with a status that is not the one `served` accepts."""
        return _answering(HTTPStatus.NO_CONTENT)
    ...
    with refusal() as failure:
        ...
        def urlopen(
            *_args: object, **_kwargs: object
            ...
            """Fail the way the index fails while the upload is landing."""
            raise failure
