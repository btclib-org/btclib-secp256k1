...
class _Clock:
    ...
    def __init__(self) -> None:
        """Start at zero, with nothing slept for yet."""
        self.now = 0.0
...
class _Transport:
    ...
    def __init__(self, answers: list[bool]) -> None:
        """Take the verdicts to hand out, oldest first."""
...
def test_an_empty_tag_is_nothing_to_wait_for(
    ...
    The clock is put in place although nothing here should sleep, and
    that is the point: without it a regressed guard falls through to the
    real `time.sleep` and the suite sits for the whole default budget
    before reporting anything, where the fake turns the same regression
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    above, and every other case here either states a deadline of its own
    or is answered before any deadline it could have been given would
    bite -- so the shipped number could be changed without a red run.
    This is the run the workflow makes.
    """
    index = _index(script, monkeypatch, [])
...
def test_a_2xx_that_is_not_ok_is_not_the_index_serving_it(
    ...
    def urlopen(_request: object, *, timeout: float) -> AbstractContextManager[_Answer]:
        """Answer with a status the index does not serve a document with."""
        assert timeout == 10.0
